...
# 非AI内容黑名单关键词，命中则过滤掉
NON_AI_KEYWORDS = [
    "手机发布", "手机评测", "发布会", "折叠屏", "充电", "摄像头",
    "oppo", "vivo", "小米", "华为手机", "苹果手机", "iphone",
    "游戏机", "显示器", "耳机", "智能手表", "电视",
    "nba", "nfl", "足球", "篮球", "体育",
    "股票", "基金", "理财", "房地产",
]
...
def is_ai_related(title):
    """预判断标题是否与AI相关，同时过滤明显非AI内容"""
    title_lower = title.lower()
    # 先检查是否命中非AI黑名单
    if any(kw in title_lower for kw in NON_AI_KEYWORDS):
        return False
...
2. 【去重合并】同一事件的多篇报道只保留最重要的1篇：
   - Google I/O、GPT发布等大事件，选最核心的1篇，其余丢弃
   - 同一公司同类型的合作公告（如多篇"XX公司与OpenAI合作"），只保留1篇

3. 【过滤低价值内容】以下内容直接丢弃，不放入日报：
   - 企业官方博客的客户案例/合作公告（如"XX航空用了我们的产品"）
   - 品牌宣传性质的内容（无实质技术或行业信息）
   - 与AI无关的科技产品发布（手机、硬件等）
   - 每个来源最多保留3条，超出部分选最重要的3条

4. 【分类标准】
   - 🤖 模型与技术：新模型发布、技术突破、研究进展、开源项目
   - 📱 应用与产品：AI产品新功能、工具发布、用户案例
   - 💰 商业与变现：融资、营收、商业模式、AI赚钱方法
   - 🏢 企业动态：公司战略、行业政策、重大合作（需有行业影响力）
   - 🎓 学习与活动：课程、活动、教程、社区动态

5. 每个分类3-8条为宜，没有内容的分类返回空数组
6. 只输出JSON，不要任何其他文字
